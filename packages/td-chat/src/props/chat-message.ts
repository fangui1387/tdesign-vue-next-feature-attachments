...
import { TdChatItemProps } from '../types';
...
export default {
  ...
  /** 自定义的操作内容（推荐使用） */
  actionbar: {
    type: [String, Function] as PropType<TdChatItemProps['actionbar']>,
  },
  ...
  /** 对话单元的内容 */
  content: {
    type: [String, Function, Array, Object] as PropType<TdChatItemProps['content']>,
  },
  /** 对话单元的时间配置 */
  datetime: {
  ...
  status: {
    ...
    validator(val: TdChatItemProps['status']): boolean {
      if (!val) return true;
  ...
  /** 对话框位置 */
  placement: {
    type: String as PropType<TdChatItemProps['placement']>,
    default: 'left' as TdChatItemProps['placement'],
    validator(val: TdChatItemProps['placement']): boolean {
      if (!val) return true;
