...
| onFileSelect | Function | - | TS 类型：`({files: FileList, name: UploadActionType}) => void`<br/>文件选择回调 | N |
...
| fileSelect | `({files: FileList, name: UploadActionType})` | 文件选择回调 |
...
| prefix | - | 输入框左下角区域扩展 |
| suffix | `{ renderPresets: UploadActionConfig[] }` | 输入框右下角区域扩展 |
