...
description: TDesign适配桌面端的AI Chat组件库，适合在 Vue 2.7 技术栈中的AI chat组件。
spline: ai
---
...
import TDesignChat from '@jump-mp/td-chat'; // 引入chat组件
import 'tdesign-vue/es/style/index.css'; // 引入少量全局样式变量
...
<div style="background: #fff5e4; display: flex; align-items: center; line-height: 20px; padding: 14px 24px; border-radius: 3px; color: #555a65;margin:16px 0">
   ⚠️ 如果按需使用的同时，需要通过主题生成器导出主题覆盖全局样式，建议请在 `main.ts` 按需注册使用 `Vue.use(TChatAction)`
</div>

## 多语言配置

`@jump-mp/td-chat` 的所有组件都内置支持了多语言配置的能力。

如果您有多语言的需求，可以通过 `tdesign-vue` 提供的[全局配置能力](https://tdesign.tencent.com/vue/components/config-provider#%E5%9B%BD%E9%99%85%E5%8C%96%E9%85%8D%E7%BD%AE)进行配置。
...
<script>
import { ConfigProvider } from '@jump-mp/td-chat/es/config-provider';
...
推荐安装 `volar`，并在项目的 tsconfig.json 的 `includes` 属性中增加`node_modules/@jump-mp/td-chat/global.d.ts`，即可实现该效果。
